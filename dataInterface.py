...
#~ 
	...
	theMasterFile = {
		'sw53X':"/home/home4/institut_1b/schomakers/FrameWork/SubmitScripts/Input/Master53X.ini",
		'sw74X':"/home/home4/institut_1b/schomakers/FrameWork/SubmitScripts/Input/Master74X_25nsMC_ReMiniAOD.ini",
	}
		...
		for i in xrange(iMax):
			if (tree.GetEntry(i) > 0):
				...
				if (weightString != ""):
					...
					#~ data.weight = tree.weight * tree.genWeight * eventWeight
				else:
					data.weight = tree.weight * weight
					#~ data.weight = tree.weight * tree.genWeight * weight
...
#~ 
		...
		#~ negWeightFraction = self.getNegWeightFraction(dataset)
		#~ kFactor = 0.0
		#~ for job in jobList:
...
#~ 
	...
	#~ def getNegWeightFraction(self, dataset, dataSamples=None):
		#~ if (dataSamples == None):
			#~ dataSamples = self.theConfigDict['DataSamples']
#~ 
		#~ if (not InfoHolder.theDataSamples.has_key(dataSamples)):
...
#~ 
		#~ jobList = (InfoHolder.theDataSamples[dataSamples])[dataset]
		#~ negWeightFraction = 0.0
		#~ for job in jobList:
			#~ negWeightFraction += float(self.cfg.get(job, 'negWeightFraction'))
#~ 
		#~ return negWeightFraction
...
#~ 
	...
	def getEventCount(self, job, flag, task, dataSamples=None):
		...
		#~ if job == "TT_Dilepton_Powheg_Spring15_25ns" or job == "WZTo2L2Q_aMCatNLO_Spring15_25ns" or job == "ZZTo2L2Q_aMCatNLO_Spring15_25ns" or "Data" in job or "HT_Run2015" in job or "T6bbllslepton" in job:
			#~ task = "cutsV29DileptonFinalTrees"
		#~ else:
			#~ task = "cutsV28DileptonFinalTrees"
